              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-lg font-heading">{room.nom}</h3>
                  </div>
                  <Badge variant="outline" className={getStatusColor(room.statut)}>{room.statut}</Badge>
                </div>
                ...
                <div className="flex items-center gap-2 border-t border-border pt-3">
                  {room.equipements.map((eq: string, i: number) => (
                    <div key={i} className="h-7 w-7 rounded-md bg-muted flex items-center justify-center text-muted-foreground" title={eq}>
                      {getEquipmentIcon(eq)}
                    </div>
                  ))}
                  {room.equipements.length === 0 && <span className="text-xs italic text-muted-foreground">Aucun équipement</span>}
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden sticky top-24">
            <div className="p-6 border-b border-border bg-gradient-to-br from-primary/10 to-transparent">
              <div className="flex justify-between items-start mb-2">
                <h2 className="text-2xl font-bold font-heading">{selectedRoom.nom}</h2>
                <Badge className={getStatusColor(selectedRoom.statut)}>{selectedRoom.statut}</Badge>
              </div>
              <p className="text-muted-foreground">{selectedRoom.type} • Capacité maximale: {selectedRoom.capacite}</p>
            </div>
